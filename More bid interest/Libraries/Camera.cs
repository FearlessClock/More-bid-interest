...
namespace More_bid_interest
{
    class Camera
    {
        ...
        //Directionel stuff
        static int upButton = 0;
        static public int GetDirButtonPress
        {
            get {
                int ret = upButton;
                upButton = 0;
        ...
        static public int HasItemBeenSelected
        {
            get {
                int ret = selectButton;
                selectButton = 0;
                return ret; }
        }

        //Select
        static int mashButton = 0;
        static double fillRate = 0;
        static double fillRateMax = 10;
        static public int HasButtonBeenMashed
        {
            get {
                if(mashButton == 1)
                {
        ...
        static public bool HasFinishedMashing(int limit)
        {
            if (fillRate >= limit)
            {
                fillRate = 0;
        ...
        public static void CheckPresses()
        {
            ...
            if (key.IsKeyDown(Key.Up) && lastKey.IsKeyUp(Key.Up))
                upButton = -1;
            else if (key.IsKeyUp(Key.Up) && lastKey.IsKeyDown(Key.Up))
                upButton = 0;
            if (key.IsKeyDown(Key.Down) && lastKey.IsKeyUp(Key.Down))
                upButton = 1;
            else if (key.IsKeyUp(Key.Down) && lastKey.IsKeyDown(Key.Down))
                upButton = 0;

            if (key.IsKeyDown(Key.Enter) && lastKey.IsKeyUp(Key.Enter))
                selectButton = 1;
            if (key.IsKeyUp(Key.Enter) && lastKey.IsKeyDown(Key.Enter))
                selectButton = 0;
